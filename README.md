這份程式碼是一套 **「基於行為數據與心理學的 AI 電商行銷 Agent」**。

它成功結合了 **資料分析 (Data Analytics)** 與 **生成式 AI (GenAI)**，能夠讀取冷冰冰的 CSV 數據，自動判讀消費者的心理狀態，並寫出具有說服力的廣告文案。

以下我將這份程式碼拆解為 **四大核心模組** 來為您介紹：

---

### 1. Agent 的大腦：策略知識庫 (`STRATEGY_BANK`)

這是 Agent 的智慧來源。我們將您提供的《消費者購物阻礙及策略》筆記，轉化為 Python 的字典結構 `[cite: 43]`。
